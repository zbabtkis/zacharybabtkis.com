...
export const metadata: Metadata = {
  ...
  description:
    'When an agent sets up a third-party service for a user, OAuth is the wrong shape for the flows that matter most. Token acquisition strategies, pooled accounts, the claim pattern, and the OAuth mechanics that bite — from the engineer who built agent auto-provisioning at ZeroClick.',
};
...
export default function OauthForAgentsPage() {
  return (
    <ArticleLayout
      title="OAuth assumes your user already has an account"
      description="Why the authorization-code flow can't start an agent-driven trial, and the token acquisition patterns that can."
      ...
      faq={[
        ...
        {
          question: 'Why not just use API keys everywhere?',
          answer:
      ...
      ctaBody="The agent-readiness audit is $3,000, credited toward follow-on work. I evaluate your codebase and auth architecture through an agent's eyes — including exactly the token acquisition, provisioning, and handover patterns this article covers — and you get a sequenced plan with a fixed quote for the build."
      ctaEmailSubject="Agent-Readiness Audit — auth and provisioning"
      ctaSource="oauth-agents-article"
    >
      <p>
        You want an agent to set up a third-party service for a user in the
        middle of a conversation. Create the project, configure it, hand
        over working credentials. The obvious answer is OAuth. Then you hit
        the assumption buried in the authorization-code flow: the user has
        to authenticate with the provider, which means the user already has
        an account there. Any flow whose value proposition is &ldquo;try
        this service without signing up first&rdquo; cannot start with
      ...
      <p>
        ...
        are what survived contact with real providers, real load balancers,
        and real users who open emails hours late.
      </p>

      <h2>OAuth is one strategy for obtaining a token</h2>
      <p>
        The unlock came from a refactor we shipped as a breaking change.
        The provisioning path originally assumed OAuth end to end.
        We changed it to accept an opaque access token and stop caring
        where the token came from. Behind that one interface, three
        acquisition strategies became possible: the user completes OAuth,
        the user pastes a platform API key, or the platform supplies
        credentials from a pool of accounts it owns. The execution code is
        identical in all three cases. Which strategy applies is
        configuration, decided per service.
      </p>
      <p>
        This sounds small. It changes what you can build. The
      ...
      <p>
        ...
        that is under its cap, increment its counter, and release the slot
        on teardown. All of that has to happen inside a single database
        ...
        you don&rsquo;t control. The pool&rsquo;s API keys are encrypted at
        rest; they are the platform&rsquo;s own credentials, and a leak
        burns every user on that account.
      </p>
      <p>
        The bug worth knowing in advance is on the release path. Ours
        released the account slot in a code path that ran even when
        teardown had failed. The slot went back into the pool while the
        project it represented was still alive, and the counter drifted
        under the true usage. Release the slot only after teardown
        succeeds. A failed teardown should hold the slot and alert.
      </p>

      <h2>Deferred ownership: the claim pattern</h2>
      <p>
        Provisioning under platform credentials creates a resource the user
        doesn&rsquo;t own. The claim pattern fixes that: let the user try
        the service, then transfer ownership when they decide to keep it.
        Two design points made ours work.
      </p>
      <p>
        First, the link you send the user is a stable URL you control, with
        an identifier for the provisioned resource — never the
        provider&rsquo;s own transfer link. Second, the provider&rsquo;s
        invite or transfer link gets generated at click time, when the user
        opens yours. Provider transfer links are typically short-lived and
        single-use. One minted at send time is dead by the time anyone
        opens the email. The click-time hop earns its keep twice over: it
        is also where you record that the user engaged and set a session
        cookie for the steps that follow.
      </p>

      <h2>Time-bound everything you provision</h2>
      <p>
        Anything created under platform credentials needs a deadline. Ours
        was a two-hour trial window before automatic teardown — cut down
        from an original fifteen days once we saw how the economics worked.
        The stored provisioning context lived slightly longer than the
        deadline itself, three hours against two, so the cleanup worker had
        a grace period to find everything it needed even if it ran late.
      </p>
      <p>
      ...
      <ul>
        <li>
          ...
          is the balancer&rsquo;s, and the derived URI stops matching. Ours
          broke exactly this way in production. Providers pin exact
        ...
        <li>
          <strong>Consume state atomically.</strong> Get-and-delete in one
          operation, so state is genuinely single-use. A read followed by a
      ...
      <p>
        A closing audit item. Scopes that are registered in the OAuth
        configuration but never enforced by any endpoint give the
        appearance of authorization without the substance. Every token
        looks scoped; nothing checks the scope. We found this in our own
        gateway and replaced it with real per-request verification. Grep
        for where scopes are validated. If the answer is nowhere, the
        consent screen is describing permissions that don&rsquo;t exist.
      </p>
      ...
      <p>
        ...
        quotas with transactional pools. Defer ownership with a claim step
        built on stable links and click-time generation. Put a deadline on
        everything provisioned under your credentials, and keep the
        deadline out of reach of retries. The agent never touches a
        credential. That combination let an agent stand up a working
        third-party service for a user who had never heard of the provider
        two minutes earlier — which is the flow OAuth alone cannot start.
      </p>
    </ArticleLayout>
  );
}
