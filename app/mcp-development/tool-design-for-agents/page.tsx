  ...
  return (
    ...
    >
      <p>
        ...
        model and watch it fail in ways your handlers never see: the model
        ignores the server entirely, calls tools in the wrong order, or
      ...
      <p>
        The highest-leverage text you write is the server-level
        instructions field, not the tool descriptions. Tool descriptions
        tell the model what a tool does once the model has already decided
        your server is relevant. The instructions decide that first
        question. Two things belong there: concrete trigger situations,
        and the order to call your tools in. The shape looks like this:
      </p>
      <pre>
...
2. activate_offer — provision the selected service`}</code>
      ...
      <p>
        Models act on examples more reliably than on category
        descriptions. &ldquo;Use this when the user wants to set up a
        service&rdquo; is weaker than three example utterances that should
        trigger the server. And naming the call order up front removes the
        most common sequencing failure: the model calling the execute tool
        cold, with an identifier it guessed.
      </p>
      ...
      <p>
        ...
        cannot hallucinate a usable option from an unusable one, because
        the unusable ones are not in its context. Any validation you can
        do server-side before returning results is validation the model
      ...
      <p>
        ...
        reliably than a nested object. Asked to reconstruct a nested
        parameter, models drop fields, rename keys, and coerce types.
        Asked to pass back a string they were given, they mostly pass back
      ...
      <p>
        ...
        require it to follow directions, which is the thing models do
        best. If your tool&rsquo;s consumer is an agent, the most useful
      ...
      <p>
        ...
        test with a real model connected. Three questions, tested
