...
export default function McpElicitationPage() {
  return (
    ...
    >
      <p>
        Say your tool call is halfway done when it hits a question only
        the human can answer. Which email address should the invite go
        to? Which region should the project deploy in? Sometimes the
        third-party provider forces the issue with an interactive step in
        the middle of an otherwise automated flow. The obvious move is to
        fail the call and let the model retry with more arguments. But
        that throws away every step already completed: accounts created,
        resources allocated, minutes of polling. The better move is to
        pause, ask, and resume.
      </p>

      <h2>Elicitation is the protocol answer</h2>
      <p>
        MCP has a mechanism for exactly this. The server sends the client
        ...
        back to the server, which picks the tool call back up.
      </p>
      <p>
        Quick version note, as of July 2026. That
        request-over-an-open-stream mechanism is how elicitation works on
        the 2025 spec versions deployed everywhere today. The MCP
        2026-07-28 spec restructures it into a stateless shape: the
        server returns an input-required result, and the client re-issues
        the call with the answers attached, so any replica can pick up
        the resume. That&rsquo;s the same suspend-and-resume design this
        article builds by hand, adopted by the protocol itself. The
        store-based pattern below gets you that behavior on the spec
        versions your clients speak today, and it survives the migration.
      </p>
      <p>
        ...
        string. Reshaping the request into MCP&rsquo;s form,{' '}
        ...
        and a native form drawn by the MCP client itself. I never wrote a
        translation layer between them. The server describes what it
        needs once, and every surface that can render a JSON Schema can
        collect it.
      </p>

      <h2>Implementing the pause</h2>
      <p>
        Now to the implementation. The shape that works is a coroutine
        suspended over an external store. The executing code calls
      ...
      <p>
        Notice that the executing code never knows it was suspended. It
        wrote <code>const email = await prompt(...)</code> and got an
        email back. All the machinery lives underneath that one awaited
        call: the store, the status transition, the polling. That&rsquo;s
        the property to preserve when you build this. The flow author
        writes straight-line code, and the runtime handles the
        suspension.
      </p>

      <h2>Machine time and human time are different budgets</h2>
      <p>
        Long-running tool calls need execution timeouts. Everything&rsquo;s
        fine until a naive timeout kills exactly the call elicitation
        exists to save. The system I built gave each execution a
        five-minute budget, which is plenty for API calls and polling
        loops and a hard stop for runaway code. Then a user takes six
        minutes to decide on a project name and blows the budget while
        the code does nothing at all.
      </p>
      <p>
        The fix is to re-arm the execution timeout every time an
        elicitation answer arrives. Machine work stays bounded at five
        minutes per stretch; human thinking time never counts against it.
        The call can wait as long as the person needs, and a hung API
        call still dies on schedule. You&rsquo;ll likely still want an
        outer deadline after which an abandoned job gets cleaned up, but
        that deadline is a product decision, and a separate knob from the
        execution budget.
      </p>

      <h2>The read-your-writes race</h2>
      <p>
        Here&rsquo;s the bug everyone building this hits. The answer
        endpoint wrote the user&rsquo;s response into the pending-request
        record. The status view the client polls was a separate
        projection, and it still reported the job as awaiting input until
        a background pass caught up. In that window the client saw an
        unanswered question, re-rendered the form, and asked the user a
      ...
      <p>
        ...
        is answered; any without is open. If you catch yourself adding a
        flag to mark answers as &ldquo;really answered,&rdquo; you have
        two sources of truth and the race is still there.
      </p>

      <h2>Two kinds of input</h2>
      <p>
        So should every question pause execution? No. Split your inputs
        into known-unknowns and unknown-unknowns.
      </p>
      <p>
        ...
        the flow itself and collect them before execution begins.
        Declaring them up front pays off three ways: missing input fails
        fast instead of minutes in, the user answers everything in one
        pass instead of being interrupted repeatedly, and the host
        application can prefill fields it already knows, skipping the
        question entirely. The system I built evolved this way. The first
        version hardcoded a single required email prompt, and later
        versions replaced it with a declared list of required inputs,
        looping over them and skipping any already present in context.
      </p>
      <p>
        Unknown-unknowns only surface mid-execution: the provider
        returned three options and someone has to pick one, or a step
        that&rsquo;s usually automatic demanded verification this time.
        These are what mid-call elicitation is for. If you find a flow
      ...
      <p>
        ...
        revision added URL-mode elicitation for exactly those flows. In
        the system I built, responding to an elicitation required a
        session token proving the caller owned the job. It took two
        validators: one for trusted server contexts, and a separate one
        for browser contexts, because a form running in a user&rsquo;s
        browser must never hold a private API key. Whatever your setup,
        the rule is the same: bind answers to the session that asked the
        question.
      </p>
      ...
      <p>
        ...
        chosen determines which projects can be named. Process
        elicitations sequentially: ask, wait, resume, and let the next
        question be computed with the previous answer in scope. Parallel
        forms look faster and produce contradictions. The up-front
        declared inputs are where you batch; the mid-flight questions are
        where you serialize.
      </p>
      ...
      <p>
        ...
        agent starts work it can&rsquo;t fully specify, the person
        supplies the missing pieces when they matter, and nothing already
        ...
        write for the answer, and a token proving who&rsquo;s answering.
        Get those five right and the rest is product design.
      </p>
    </ArticleLayout>
  );
}
