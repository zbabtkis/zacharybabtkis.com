...
export const metadata: Metadata = {
  ...
  description:
    'A VM context isolates scope, not capability. What it takes to run partner, user, or model-generated code server-side: SSRF and DNS rebinding defenses, log exfiltration, timeouts, and the execution contract — lessons from a system I led at ZeroClick.',
};
  ...
  return (
    ...
    >
      ...
      <p>
        ...
        point. Our first version gave the sandbox the platform&rsquo;s own{' '}
      ...
      <p>
        ...
        partners, not anonymous users — a threat-model call, and one you
        should make explicitly rather than by default.
      </p>

      <h2>The execution contract</h2>
      <p>
        Untrusted code should conform to your shape, not the other way
        around. Our integrations were a fixed set of named lifecycle
        ...
        Nothing ambient — no <code>require</code>, no <code>process</code>,
        no filesystem. The discipline that pays off: enumerate exactly
        what you inject, and treat that list as the security review. If
      ...
      <p>
        ...
        credentials server-side — never route them through the caller. In
      ...
      <p>
        ...
        secrets, full stop, and results are fetched separately with a
        one-time, caller-bound token that is consumed atomically on use.
        Worth noting: our first attempt at &ldquo;one-time&rdquo; minted a
        fresh token on every poll, which meant an unbounded supply of
        valid tokens and defeated the point. One-time means one token,
        used once, gone.
      </p>
      ...
      <ol>
        <li>
          ...
          your attack surface — review it, not the sandboxed code.
        </li>
        <li>
      ...
      <p>
        None of these bugs required a sophisticated attacker. Each one was
        an ordinary engineering decision — hand over fetch, pipe the
        console, open the status endpoint — that was reasonable in
        isolation and wrong at the boundary. We caught them in-house
